      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 5:
            user = _context.sent;
            // 接下来可能写一波的注释解析一下这段代码
            secret = __WEBPACK_IMPORTED_MODULE_6__config__["a" /* default */].jwt.secret; // jwt 类似的一个秘钥

            expiresIn = __WEBPACK_IMPORTED_MODULE_6__config__["a" /* default */].jwt.expiresIn;
            token = __WEBPACK_IMPORTED_MODULE_2_jsonwebtoken___default.a.sign({ username: user.username, userID: user._id }, secret); // token 需要有效载荷 和 秘钥
